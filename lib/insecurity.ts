...
const privateKey = fs ? fs.readFileSync('encryptionkeys/jwt.pem', 'utf8') : 'placeholder-private-key'
...
const normalizeRedirectUrl = (url: string) => {
  try {
...
export const getAllowedRedirectUrl = (url: string) => {
  const normalizedUrl = normalizeRedirectUrl(url)
  if (!normalizedUrl) {
    return undefined
  }
  for (const allowedUrl of redirectAllowlist) {
    if (normalizedUrl === allowedUrl) {
      return allowedUrl
    }
  }
  return undefined
...
export const isRedirectAllowed = (url: string) => {
  return getAllowedRedirectUrl(url) !== undefined
}
